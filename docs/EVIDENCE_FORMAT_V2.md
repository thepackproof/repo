...
This specification defines the evidence manifest produced by PackProof 0.3.0. It is an implementation contract, not a certification, a physical-authenticity result, or a legal conclusion.
...
PackProof 0.3.0 reports physical correspondence as `NOT_AVAILABLE` with reason `NO_VALIDATED_PHYSICAL_MATCHER_ENABLED`. It does not produce a production `MATCH` or `NON_MATCH` result.
