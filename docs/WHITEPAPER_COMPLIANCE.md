...
| PHY-10 | **5.7 Three-way decision rule** | Implement frozen `MATCH`, `NON_MATCH`, and `INCONCLUSIVE` thresholds, with separate FTA, quality, eligibility, and policy states. Do not label raw similarity as probability. | Exact threshold-boundary, floating-point, quality-failure, and unsupported-state tests pass. |
...
- Three-way rule: `S <= T_nonmatch` is non-match; `T_nonmatch < S < T_match` is inconclusive; `S >= T_match` is match, only after eligibility, quality, and coverage pass.
