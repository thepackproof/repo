...
These claims intentionally describe documentation, not authentication or guaranteed dispute outcomes.
...
Create a shared record of item condition, terms, packing and delivery.

## Full description

PackProof helps buyers and sellers document high-value private transactions in one shared, private record.

Create a transaction passport with the item description, agreed price, condition notes, identifiers, certificates and exact sale terms. Invite the other participant with a one-use link, then lock the shared terms only after both people confirm them.

Capture fulfillment as it happens:
...
- Background: deep navy `#07111E`.
- Primary accent: proof teal `#21D4B4`.
- Secondary accent: signal blue `#5B8CFF`.
- Motif: a sealed parcel formed by a continuous checkmark line.
- Feature graphic phrase: “A shared record from agreement to arrival.”
- Use original product mockups and generic collectibles; do not show third-party marketplace or luxury-brand trademarks without permission.

Ready-to-upload starters are included at `assets/store/play-icon-512.png` and `assets/store/feature-graphic-1024x500.png`. Capture phone screenshots from the final Play internal-test build so they accurately show the released UI and configured prices.
