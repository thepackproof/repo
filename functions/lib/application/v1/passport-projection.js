...
exports.passportArtifactInput = passportArtifactInput;
...
exports.canonicalPassportDigest = canonicalPassportDigest;
exports.nextSnapshot = nextSnapshot;
exports.looksLikePassportIdentity = looksLikePassportIdentity;
...
function passportArtifactInput(record) {
    ...
    const finalization = record.serverFinalized || record.serverVerified
        ? integrityFailed ? 'QUARANTINED' : 'FINALIZED'
        : record.sha256 ? 'UPLOADED' : 'RESERVED';
    return {
        id: record.id,
...
function passportTransactionInput(transaction) {
    return {
        id: transaction.id,
...
function boundOrIssuedIdentity(transaction, issuedAt) {
    ...
    const issued = (0, passport_1.issuePassportIdentity)(transaction.id);
    return { ...issued, issuedAt, bind: true };
}
function snapshotDto(record) {
    return {
...
function exportDto(record, urls) {
    if (!record.pdfSha256)
        throw new errors_1.ApplicationError('FAILED_PRECONDITION', 'PASSPORT_EXPORT_NOT_READY', 'This snapshot does not have a presentation export yet.');
    return {
...
function canonicalPassportDigest(passport) {
    return (0, evidence_format_1.sha256Hex)((0, evidence_format_1.canonicalizeJson)(passport));
}
function nextSnapshot(passport, version, generatedAt) {
    return {
        snapshotId: (0, passport_1.issuePassportSnapshotId)(passport.identity.passportId, version),
        ...
        rendererVersion: passport_1.PASSPORT_PDF_RENDERER_VERSION,
        generatedAt,
        pdfStoragePath: null,
        pdfSha256: null,
    };
}
function looksLikePassportIdentity(value) {
    return (0, passport_1.isPassportResourceId)(value) || (0, passport_1.isPassportDisplayId)(value);
}
//# sourceMappingURL=passport-projection.js.map