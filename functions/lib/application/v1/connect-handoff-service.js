...
function evaluateConnectGrantExchange(session, command, tokenVerifier, transactionId, now) {
    if (!session)
        throw new errors_1.ApplicationError('NOT_FOUND', 'CONNECT_SESSION_NOT_FOUND', 'PackProof API session not found.');
    if (session.claimedBy && session.claimedBy !== command.actorId) {
        throw new errors_1.ApplicationError('CONFLICT', 'CONNECT_SESSION_ALREADY_CLAIMED', 'This PackProof API session was claimed by another account.');
    }
    ...
    if (session.expiresAt.getTime() < now.getTime()) {
        throw new errors_1.ApplicationError('DEADLINE_EXCEEDED', 'CONNECT_SESSION_EXPIRED', 'PackProof API session expired.');
    }
    if (session.status !== 'PENDING_REDEMPTION') {
        throw new errors_1.ApplicationError('FAILED_PRECONDITION', 'CONNECT_SESSION_NOT_REDEEMABLE', 'This PackProof API session cannot be redeemed in its current state.');
    }
    ...
    if (!session.tokenHash || !tokenVerifier.verify(command.token, session.tokenHash)) {
        throw new errors_1.ApplicationError('FORBIDDEN', 'INVALID_HANDOFF_TOKEN', 'Invalid PackProof API handoff token.');
    }
    const terms = {
        saleType: 'SHIPPED',
