...
exports.PASSPORT_PDF_PAGE_WIDTH = 612;
exports.PASSPORT_PDF_PAGE_HEIGHT = 792;
const STILL_ORDER = ['ITEM', 'INTERIOR', 'SEAL', 'LABEL'];
function stillRoleForArtifact(type, contentType) {
    const media = (contentType ?? '').toLowerCase();
    if (media.startsWith('video/'))
        return null;
    if (type === 'ITEM_PHOTO' || type === 'IDENTIFIER_PHOTO' || type === 'CONDITION_PHOTO' || type === 'COA_PHOTO')
        return 'ITEM';
    if (type === 'PHYSICAL_REFERENCE_FRAME')
        return 'INTERIOR';
    if (type === 'STATION_SEAL_REFERENCE')
        return 'SEAL';
    if (type === 'SHIPPING_LABEL' || type === 'RETURN_SHIPPING_LABEL')
        return 'LABEL';
    return null;
}
function passportQrMatrix(payload) {
...
function passportPdfPagePlan(passport, stills = []) {
    const pages = ['COVER'];
    if (stills.length || passport.fulfillment.packingArtifactId)
        pages.push('STILLS');
    pages.push('COMPARISON');
    if (passport.shipment || passport.delivery || passport.fulfillment.trackingObserved.value)
        pages.push('SHIPMENT');
...
class PassportPdfWriter {
    ...
    muted;
    rule;
    constructor(pdf, regular, bold) {
        this.pdf = pdf;
        this.regular = regular;
        this.bold = bold;
        this.dark = (0, pdf_lib_1.rgb)(0.07, 0.09, 0.12);
        this.teal = (0, pdf_lib_1.rgb)(0.27, 0.49, 0.39);
        this.muted = (0, pdf_lib_1.rgb)(0.36, 0.40, 0.47);
        this.rule = (0, pdf_lib_1.rgb)(0.82, 0.85, 0.88);
    }
    startPage(overflow = false) {
        this.page = this.pdf.addPage([exports.PASSPORT_PDF_PAGE_WIDTH, exports.PASSPORT_PDF_PAGE_HEIGHT]);
    ...
    drawQr(payload, x, y, size) {
        const matrix = passportQrMatrix(payload);
        const cell = size / matrix.size;
        this.page.drawRectangle({ x, y, width: size, height: size, color: (0, pdf_lib_1.rgb)(1, 1, 1) });
...
async function embedStill(pdf, still) {
    try {
        if (still.contentType.toLowerCase().includes('png'))
            return await pdf.embedPng(still.bytes);
...
async function renderPassportPdf(passport, stills = []) {
    ...
    for (const pageId of pages) {
        ...
        if (pageId === 'COVER') {
            ...
            writer.line(passport.integrity.banner === 'AUTHENTIC_PACKPROOF' ? passport_1.INTEGRITY_MEANING_VERIFIED : passport_1.INTEGRITY_MEANING_LIMITED, { gap: 8 });
            ...
            writer.line(passport_1.COMPARISON_FOOTNOTE_COPY, { size: 8, color: writer.muted, gap: 4 });
            ...
            writer.line(passport_1.PASSPORT_PAGE_ONE_FOOTER, { size: 8, color: writer.muted, gap: 2 });
            ...
            writer.line(passport_1.COMPARISON_FOOTNOTE_COPY, { size: 8, color: writer.muted, gap: 8 });
            for (const comparison of passport.items[0]?.comparisons ?? []) {
            ...
            writer.line(`Renderer: ${passport_1.PASSPORT_PDF_RENDERER_VERSION}`);
            ...
            if (!passport.artifacts.length)
                writer.line('No artifacts were present.');
...
//# sourceMappingURL=passport-pdf.js.map