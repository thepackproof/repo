...
exports.unregisterPushToken = exports.registerPushToken = exports.blockUser = exports.raiseConcern = exports.completeTransaction = exports.confirmLocalHandoff = exports.markReceived = exports.submitShipping = exports.requestEvidenceUpload = exports.confirmTerms = exports.acceptInvite = exports.createInvite = exports.cancelTransaction = exports.saveTransactionDraft = exports.ensureUserProfile = void 0;
const node_crypto_1 = require("node:crypto");
const firestore_1 = require("firebase-admin/firestore");
const https_1 = require("firebase-functions/v2/https");
const consumer_transaction_service_1 = require("./application/v1/consumer-transaction-service");
...
const consumerTransactionService = new consumer_transaction_service_1.ConsumerTransactionApplicationService(new consumer_transaction_repository_1.FirestoreConsumerTransactionRepository(config_1.db));
async function withStorageStatus(grant, expiresAt) {
    if (grant.status === 'FINALIZED')
        return { ...grant, expiresAt: expiresAt.toDate().toISOString() };
    const [exists] = await config_1.storage.bucket().file(grant.storagePath).exists();
    return {
        ...grant,
        status: exists ? 'PROCESSING' : 'READY',
        expiresAt: expiresAt.toDate().toISOString(),
    };
}
function verifyDeviceKeyProof(proof, nonce) {
    try {
        const publicKey = (0, node_crypto_1.createPublicKey)({ key: Buffer.from(proof.publicKeySpkiBase64, 'base64'), format: 'der', type: 'spki' });
        return (0, node_crypto_1.verify)('sha256', Buffer.from(nonce, 'utf8'), publicKey, Buffer.from(proof.challengeSignatureBase64, 'base64'));
    }
    catch {
        return false;
    }
...
exports.ensureUserProfile = (0, https_1.onCall)(callOptions, async (request) => {
    const uid = (0, helpers_1.requireUid)(request);
    const authUser = await config_1.adminAuth.getUser(uid);
    const ref = config_1.db.collection('users').doc(uid);
    ...
    const data = {
        ...(0, helpers_1.publicUser)(authUser),
        providers,
        updatedAt: firestore_1.FieldValue.serverTimestamp(),
        ...(snap.exists ? {} : { createdAt: firestore_1.FieldValue.serverTimestamp(), plan: 'FREE', moderationState: 'ACTIVE' }),
    };
    await ref.set(data, { merge: true });
    await config_1.db.collection('publicProfiles').doc(uid).set({ uid, displayName: data.displayName, photoURL: data.photoURL, updatedAt: firestore_1.FieldValue.serverTimestamp() }, { merge: true });
    return {
        ...(0, helpers_1.publicUser)(authUser),
        providers,
...
exports.saveTransactionDraft = (0, https_1.onCall)(callOptions, async (request) => {
    const uid = (0, helpers_1.requireUid)(request);
    const profile = await (0, helpers_1.assertAccountActive)(uid);
    const input = validation_1.transactionDraftSchema.parse(request.data);
    try {
...
exports.cancelTransaction = (0, https_1.onCall)(callOptions, async (request) => {
    const uid = (0, helpers_1.requireUid)(request);
    ...
    const hadBuyer = await config_1.db.runTransaction(async (tx) => {
        const snap = await tx.get(ref);
        if (!snap.exists)
            throw new https_1.HttpsError('not-found', 'Transaction not found.');
        const record = snap.data();
        (0, helpers_1.assertSeller)(record, uid);
        if (!['DRAFT', 'AWAITING_BUYER', 'TERMS_REVIEW'].includes(record.status)) {
            throw new https_1.HttpsError('failed-precondition', 'Only an unlocked PackProof can be cancelled.');
        }
        if (record.activeInviteHash)
            tx.delete(config_1.db.collection('invites').doc(record.activeInviteHash));
        tx.update(ref, {
            status: 'CANCELLED',
            confirmedBy: [],
            lockedAt: null,
            activeInviteHash: firestore_1.FieldValue.delete(),
            updatedAt: firestore_1.FieldValue.serverTimestamp(),
        });
        return Boolean(record.buyerId);
    });
    await (0, helpers_1.appendEvent)(transactionId, uid, 'TRANSACTION_CANCELLED', 'Seller cancelled the PackProof before the terms were locked.');
    if (hadBuyer)
        await (0, helpers_1.notifyOtherParticipants)(transactionId, uid, 'PackProof cancelled', 'The seller cancelled this PackProof before the terms were locked.');
    return { success: true };
});
exports.createInvite = (0, https_1.onCall)(callOptions, async (request) => {
    const uid = (0, helpers_1.requireUid)(request);
    await (0, helpers_1.assertAccountActive)(uid);
    const { transactionId } = validation_1.transactionIdSchema.parse(request.data);
    const { ref, data } = await (0, helpers_1.getTransaction)(transactionId);
    (0, helpers_1.assertSeller)(data, uid);
    if (data.buyerId)
        throw new https_1.HttpsError('already-exists', 'A buyer has already joined this transaction.');
    if (data.status === 'CANCELLED' || data.status === 'ARCHIVED')
        throw new https_1.HttpsError('failed-precondition', 'This transaction cannot be invited.');
    const code = (0, helpers_1.randomToken)(32);
    const codeHash = (0, helpers_1.hash)(code);
    const inviteRef = config_1.db.collection('invites').doc(codeHash);
    await config_1.db.runTransaction(async (tx) => {
        const fresh = (await tx.get(ref)).data();
        if (!fresh)
            throw new https_1.HttpsError('not-found', 'Transaction not found.');
        (0, helpers_1.assertSeller)(fresh, uid);
        if (fresh.buyerId)
            throw new https_1.HttpsError('already-exists', 'A buyer has already joined this transaction.');
        if (fresh.status === 'CANCELLED' || fresh.status === 'ARCHIVED')
            throw new https_1.HttpsError('failed-precondition', 'This transaction cannot be invited.');
        if (fresh.activeInviteHash)
            tx.delete(config_1.db.collection('invites').doc(fresh.activeInviteHash));
        tx.set(inviteRef, { transactionId, sellerId: uid, createdAt: firestore_1.FieldValue.serverTimestamp(), expiresAt: (0, helpers_1.expiresIn)(7 * 86400) });
        tx.update(ref, { status: 'AWAITING_BUYER', activeInviteHash: codeHash, updatedAt: firestore_1.FieldValue.serverTimestamp() });
    });
    await (0, helpers_1.appendEvent)(transactionId, uid, 'INVITE_CREATED', 'Seller created a private buyer invitation.');
    const url = `${config_1.publicAppUrl.value().replace(/\/$/, '')}/invite/?code=${encodeURIComponent(code)}`;
    return { code, url, expiresAt: new Date(Date.now() + 7 * 86400_000).toISOString() };
});
exports.acceptInvite = (0, https_1.onCall)(callOptions, async (request) => {
    const uid = (0, helpers_1.requireUid)(request);
    await (0, helpers_1.assertAccountActive)(uid);
    const { code } = validation_1.inviteCodeSchema.parse(request.data);
    const inviteRef = config_1.db.collection('invites').doc((0, helpers_1.hash)(code));
    const result = await config_1.db.runTransaction(async (tx) => {
        const inviteSnap = await tx.get(inviteRef);
        if (!inviteSnap.exists)
            throw new https_1.HttpsError('not-found', 'This invitation is invalid or has already been used.');
        const invite = inviteSnap.data();
        if (invite.expiresAt.toMillis() < Date.now())
            throw new https_1.HttpsError('deadline-exceeded', 'This invitation has expired.');
        if (invite.sellerId === uid)
            throw new https_1.HttpsError('failed-precondition', 'The seller cannot accept their own invitation.');
        const transactionRef = config_1.db.collection('transactions').doc(invite.transactionId);
        const transactionSnap = await tx.get(transactionRef);
        if (!transactionSnap.exists)
            throw new https_1.HttpsError('not-found', 'Transaction not found.');
        const record = transactionSnap.data();
        if (record.buyerId && record.buyerId !== uid)
            throw new https_1.HttpsError('already-exists', 'Another buyer has already joined.');
        const blockedEitherWay = await Promise.all([
            tx.get(config_1.db.collection('users').doc(uid).collection('blocks').doc(record.sellerId)),
            tx.get(config_1.db.collection('users').doc(record.sellerId).collection('blocks').doc(uid)),
        ]);
        if (blockedEitherWay.some((snap) => snap.exists))
            throw new https_1.HttpsError('permission-denied', 'This invitation is unavailable.');
        tx.update(transactionRef, { buyerId: uid, participantIds: [record.sellerId, uid], status: 'TERMS_REVIEW', confirmedBy: [], activeInviteHash: firestore_1.FieldValue.delete(), updatedAt: firestore_1.FieldValue.serverTimestamp() });
        tx.delete(inviteRef);
...
exports.confirmTerms = (0, https_1.onCall)(callOptions, async (request) => {
    ...
    const { transactionId } = validation_1.transactionIdSchema.parse(request.data);
    const ref = config_1.db.collection('transactions').doc(transactionId);
    const locked = await config_1.db.runTransaction(async (tx) => {
        const snap = await tx.get(ref);
        if (!snap.exists)
            throw new https_1.HttpsError('not-found', 'Transaction not found.');
        const record = snap.data();
        (0, helpers_1.assertParticipant)(record, uid);
        if (!record.buyerId)
            throw new https_1.HttpsError('failed-precondition', 'A buyer must join before terms can be confirmed.');
        if (!['TERMS_REVIEW', 'TERMS_LOCKED'].includes(record.status))
            throw new https_1.HttpsError('failed-precondition', 'Terms cannot be confirmed in this state.');
        ...
        tx.update(ref, {
            ...
            lockedAt: bothConfirmed ? firestore_1.FieldValue.serverTimestamp() : null,
            updatedAt: firestore_1.FieldValue.serverTimestamp(),
        });
...
exports.requestEvidenceUpload = (0, https_1.onCall)(uploadCallOptions, async (request) => {
    const uid = (0, helpers_1.requireUid)(request);
    await (0, helpers_1.assertAccountActive)(uid);
    const input = validation_1.uploadRequestSchema.parse(request.data);
    const requestFingerprint = (0, evidence_format_1.sha256Hex)((0, evidence_format_1.canonicalizeJson)({
    ...
    if (['COMPLETED', 'CANCELLED', 'ARCHIVED'].includes(data.status) && !input.returnPassportId) {
        throw new https_1.HttpsError('failed-precondition', 'This transaction no longer accepts outbound evidence.');
    }
    const sellerOnly = ['ITEM_PHOTO', 'CONDITION_PHOTO', 'IDENTIFIER_PHOTO', 'COA_PHOTO', 'PACKING_VIDEO', 'SHIPPING_LABEL', 'PHYSICAL_REFERENCE_FRAME'];
    const buyerOnly = ['UNBOXING_VIDEO', 'DELIVERY_PHOTO', 'PHYSICAL_VERIFICATION_FRAME'];
    if (sellerOnly.includes(input.evidenceType) && data.sellerId !== uid)
        throw new https_1.HttpsError('permission-denied', 'Only the seller may upload that evidence type.');
    if (buyerOnly.includes(input.evidenceType) && data.buyerId !== uid)
        throw new https_1.HttpsError('permission-denied', 'Only the buyer may upload that evidence type.');
    if (['PACKING_VIDEO', 'SHIPPING_LABEL', 'UNBOXING_VIDEO'].includes(input.evidenceType) && !['TERMS_LOCKED', 'PACKED', 'SHIPPED', 'BUYER_REVIEW', 'DISPUTED'].includes(data.status)) {
        throw new https_1.HttpsError('failed-precondition', 'Both parties must lock the terms before fulfillment evidence is captured.');
    }
    if (input.evidenceType === 'DELIVERY_PHOTO') {
        ...
        if (!allowed.includes(data.status)) {
            throw new https_1.HttpsError('failed-precondition', 'Arrival package observations can be recorded after the item is marked shipped, or during a local handoff.');
        }
    }
    let returnPassport = null;
    if (input.evidenceType.startsWith('RETURN_')) {
        if (!input.returnPassportId)
            throw new https_1.HttpsError('invalid-argument', 'Return evidence requires a return passport.');
        ...
        if (!returnData.participantIds.includes(uid))
            throw new https_1.HttpsError('permission-denied', 'You are not part of this return passport.');
        ...
        if (['RETURN_PACKING_VIDEO', 'RETURN_SHIPPING_LABEL'].includes(input.evidenceType) && returningParticipantId !== uid) {
            throw new https_1.HttpsError('permission-denied', 'Only the returning participant can add repacking or return-label evidence.');
        }
        if (input.evidenceType === 'RETURN_UNBOXING_VIDEO' && returnData.recipientId !== uid) {
            throw new https_1.HttpsError('permission-denied', 'Only the return recipient can record the returned-item unboxing.');
        }
        if (['CANCELLED', 'COMPLETED'].includes(String(returnData.status)))
            throw new https_1.HttpsError('failed-precondition', 'This return passport no longer accepts evidence.');
        if (input.evidenceType === 'RETURN_PACKING_VIDEO' && !['AUTHORIZED', 'PACKED'].includes(String(returnData.status))) {
            throw new https_1.HttpsError('failed-precondition', 'The return must be authorized before repacking.');
        }
        if (input.evidenceType === 'RETURN_SHIPPING_LABEL' && !['AUTHORIZED', 'PACKED'].includes(String(returnData.status))) {
            throw new https_1.HttpsError('failed-precondition', 'The return must be authorized before a seal reference can be recorded.');
        }
        if (input.evidenceType === 'RETURN_UNBOXING_VIDEO' && !['IN_TRANSIT', 'RECEIVED_REVIEW'].includes(String(returnData.status))) {
            throw new https_1.HttpsError('failed-precondition', 'The return must be in transit before unboxing.');
        }
    }
    else if (input.returnPassportId) {
        throw new https_1.HttpsError('invalid-argument', 'Return passport IDs may be used only with return evidence types.');
    }
    ...
    const physicalProfile = input.manifest && 'physicalCaptureProfile' in input.manifest
        ? input.manifest.physicalCaptureProfile
        : null;
    if (isPhysicalFrame && !physicalProfile) {
        throw new https_1.HttpsError('failed-precondition', 'Physical correspondence frames require the frozen physical capture profile in a v2 manifest.');
    }
    if (!isPhysicalFrame && physicalProfile) {
        throw new https_1.HttpsError('failed-precondition', 'The physical capture profile may be used only for physical correspondence frame evidence.');
    }
    if (input.evidenceType === 'PHYSICAL_REFERENCE_FRAME' && physicalProfile?.intendedUse !== 'REFERENCE') {
        throw new https_1.HttpsError('failed-precondition', 'Reference evidence must use a REFERENCE physical capture profile.');
    }
    if (input.evidenceType === 'PHYSICAL_VERIFICATION_FRAME' && physicalProfile?.intendedUse !== 'VERIFICATION') {
        throw new https_1.HttpsError('failed-precondition', 'Verification evidence must use a VERIFICATION physical capture profile.');
    }
    let captureSessionRef = null;
    let boundEvidenceSessionRef = null;
    let attestationSnapshot = null;
    if (input.captureSessionId) {
        captureSessionRef = config_1.db.collection('captureSessions').doc(input.captureSessionId);
        const captureSessionSnap = await captureSessionRef.get();
        const captureSession = captureSessionSnap.data();
        const redemptionExpiresAt = captureSession?.redemptionExpiresAt;
        const expired = !captureSessionSnap.exists || !redemptionExpiresAt || redemptionExpiresAt.toMillis() < Date.now();
        if (expired)
            throw new https_1.HttpsError('failed-precondition', 'Capture attestation session expired.');
        ...
        if (sessionMode === 'SINGLE' && captureSession?.usedAt && captureSession?.requestFingerprint !== requestFingerprint) {
            throw new https_1.HttpsError('failed-precondition', 'This attested capture is already bound to different evidence.');
        }
        if (captureSession?.uid !== uid || captureSession?.transactionId !== input.transactionId)
            throw new https_1.HttpsError('permission-denied', 'Capture attestation session mismatch.');
        if (typeof captureSession?.evidenceSessionId === 'string' && captureSession.evidenceSessionId) {
            boundEvidenceSessionRef = config_1.db.collection('evidenceSessions').doc(captureSession.evidenceSessionId);
            ...
            if (!evidenceSession.exists || evidenceSessionData?.status !== 'CAPTURING'
                ...
                throw new https_1.HttpsError('failed-precondition', 'The actor-bound evidence session is no longer active.');
            }
        }
        if ((captureSession?.returnPassportId ?? null) !== (input.returnPassportId ?? null))
            throw new https_1.HttpsError('permission-denied', 'Return attestation context mismatch.');
        if ((captureSession?.connectSessionId ?? null) !== (input.connectSessionId ?? null))
            throw new https_1.HttpsError('permission-denied', 'Connect attestation context mismatch.');
        if (input.manifest?.attestation.captureSessionId !== input.captureSessionId || input.manifest.attestation.nonce !== captureSession?.nonce) {
            throw new https_1.HttpsError('failed-precondition', 'The signed capture nonce does not match the attested session.');
        }
        ...
        if (Array.isArray(allowedEvidenceTypes) && !allowedEvidenceTypes.includes(input.evidenceType)) {
            throw new https_1.HttpsError('permission-denied', 'This evidence type is outside the actor-bound evidence session authorization.');
        }
        const issuedAt = captureSession?.issuedAt;
        const captureWindowEndsAt = captureSession?.captureWindowEndsAt;
        ...
        if (!input.manifest || input.manifest.attestation.mode !== 'JIT_APP_CHECK' || !issuedAt || !captureWindowEndsAt) {
            throw new https_1.HttpsError('failed-precondition', 'Attested uploads require a complete capture manifest.');
        }
        if (manifestStartedAt < issuedAt.toMillis() - 30_000 || manifestStartedAt > captureWindowEndsAt.toMillis()) {
            throw new https_1.HttpsError('failed-precondition', 'Recording did not begin inside the attested capture window.');
        }
        if (manifestFinishedAt < manifestStartedAt || manifestFinishedAt - manifestStartedAt > 16 * 60_000) {
            throw new https_1.HttpsError('failed-precondition', 'Capture duration is inconsistent with the allowed recording window.');
        }
        if (captureSession?.runtimeArtifactHash && input.manifest.runtimeIntegrity.runtimeArtifactHash !== captureSession.runtimeArtifactHash) {
            throw new https_1.HttpsError('failed-precondition', 'Runtime integrity fingerprint changed after attestation.');
        }
        if (sessionMode === 'BATCH') {
            if (input.manifest.attestation.sessionMode !== 'BATCH'
                ...
                throw new https_1.HttpsError('failed-precondition', 'Batch attestation context changed after the session was issued.');
            }
            if (!physicalProfile || captureSession?.captureProfileId !== physicalProfile.profileId
                ...
                throw new https_1.HttpsError('failed-precondition', 'Physical capture profile or group changed after batch attestation.');
            }
            if (!isPhysicalFrame) {
                throw new https_1.HttpsError('failed-precondition', 'This batch capture session is reserved for physical correspondence frames.');
            }
        }
        else if (isPhysicalFrame) {
            throw new https_1.HttpsError('failed-precondition', 'Physical correspondence frames require a batch capture session.');
        }
        const deviceKeyProof = input.manifest.attestation.deviceKeyProof;
        ...
        if (deviceKeyProof && !deviceKeySignatureValid) {
            throw new https_1.HttpsError('failed-precondition', 'Device-key signature did not verify against the attested nonce.');
        }
        attestationSnapshot = {
            mode: 'JIT_APP_CHECK',
        ...
        throw new https_1.HttpsError('failed-precondition', 'JIT-attested manifests require a valid capture session.');
    }
    ...
    const expectedTrackingNumber = normalizeTracking(returnPassport?.shipping?.trackingNumber ?? data.shipping?.trackingNumber ?? data.source?.trackingNumber);
    const shippingLabel = input.manifest?.shippingLabel;
    const trackerObservation = scannedTrackingNumber && shippingLabel
        ? (() => {
            const identification = (0, shipping_tracker_1.identifyTrackingNumber)(typeof shippingLabel.rawDecodedValue === 'string' ? shippingLabel.rawDecodedValue : scannedTrackingNumber, scannedTrackingNumber);
            const stillSha256 = typeof shippingLabel.still?.sha256 === 'string' ? shippingLabel.still.sha256 : null;
            const observationSha256 = (0, shipping_tracker_1.hashShippingObservation)({
            ...
            return (0, shipping_tracker_1.asShippingTrackerObservation)({
                ...
                interpretation: shipping_tracker_1.SHIPPING_OBSERVATION_INTERPRETATION,
            });
        })()
        : null;
    ...
    const ingressNetwork = {
        ipSubnetHmac: (0, node_crypto_1.createHmac)('sha256', config_1.manifestSigningSecret.value()).update(`v1\n${ingressSubnet}`).digest('hex'),
        privacyPrefix: ingressSubnet === 'unavailable' ? 'UNAVAILABLE' : ingressSubnet.endsWith('/24') ? 'IPV4_24' : 'IPV6_64',
        source: 'FUNCTION_INGRESS',
    };
    const grantExpiresAt = (0, helpers_1.expiresIn)(6 * 3600);
    const pendingBase = {
        transactionId: input.transactionId,
        uploaderId: uid,
        clientEvidenceId: input.clientEvidenceId ?? null,
    ...
    if (captureSessionRef) {
        const grant = await config_1.db.runTransaction(async (tx) => {
            ...
            const freshRedemptionExpiresAt = session?.redemptionExpiresAt;
            ...
            if ((session?.returnPassportId ?? null) !== (input.returnPassportId ?? null)
                ...
                throw new https_1.HttpsError('permission-denied', 'Capture attestation context mismatch.');
            }
            ...
            if (freshSessionMode === 'BATCH') {
                if (!physicalProfile
                    ...
                    throw new https_1.HttpsError('failed-precondition', 'Batch capture context changed before the evidence grant was bound.');
                }
                const fingerprints = Array.isArray(session?.requestFingerprints)
                    ? session.requestFingerprints.filter((value) => typeof value === 'string')
                    : [];
                const bindings = session?.uploadBindings && typeof session.uploadBindings === 'object'
                    ? session.uploadBindings
                    : {};
                const frameBindings = session?.frameBindings && typeof session.frameBindings === 'object'
                    ? session.frameBindings
                    : {};
                const existingUploadId = bindings[requestFingerprint];
                if (existingUploadId) {
                    const storagePath = `evidence/${input.transactionId}/${uid}/${existingUploadId}`;
                    const evidenceRef = config_1.db.collection('transactions').doc(input.transactionId).collection('evidence').doc(existingUploadId);
                    const pendingRef = config_1.db.collection('pendingUploads').doc(existingUploadId);
                    ...
                    if (evidenceSnap.exists) {
                        if (evidenceSnap.data()?.requestFingerprint !== requestFingerprint) {
                            throw new https_1.HttpsError('failed-precondition', 'The finalized batch frame identifier is bound to different evidence.');
                        }
                        return { uploadId: existingUploadId, storagePath, status: 'FINALIZED' };
                    }
                    if (pendingSnap.exists && pendingSnap.data()?.requestFingerprint !== requestFingerprint) {
                        throw new https_1.HttpsError('failed-precondition', 'The reserved evidence grant does not match this batch frame.');
                    }
                    if (pendingSnap.exists) {
                        tx.update(pendingRef, { reissuedAt: firestore_1.FieldValue.serverTimestamp(), expiresAt: grantExpiresAt });
                    }
                    else {
                    ...
                    return { uploadId: existingUploadId, storagePath, status: 'READY' };
                }
                const maxEvidenceCount = Math.min(Math.max(Number(session?.maxEvidenceCount ?? 1), 1), 24);
                if (fingerprints.length >= maxEvidenceCount) {
                    throw new https_1.HttpsError('resource-exhausted', 'This attested capture batch has reached its evidence limit.');
                }
                ...
                if (!frameKey || frameIndex === undefined || frameBindings[frameKey]) {
                    throw new https_1.HttpsError('failed-precondition', 'This physical capture frame position is missing or already bound.');
                }
                const uploadId = input.clientEvidenceId
                    ? (0, evidence_format_1.deterministicUploadId)({ transactionId: input.transactionId, uploaderId: uid, clientEvidenceId: input.clientEvidenceId })
                    : config_1.db.collection('pendingUploads').doc().id;
                const pendingRef = config_1.db.collection('pendingUploads').doc(uploadId);
                const evidenceRef = config_1.db.collection('transactions').doc(input.transactionId).collection('evidence').doc(uploadId);
                ...
                if (evidenceSnap.exists || pendingSnap.exists) {
                    ...
                    if (existingFingerprint !== requestFingerprint) {
                        throw new https_1.HttpsError('failed-precondition', 'The deterministic evidence identifier is already bound to different evidence.');
                    }
                    throw new https_1.HttpsError('failed-precondition', 'Batch binding state is incomplete for an existing evidence identifier.');
                }
                tx.create(pendingRef, {
                    ...pendingBase,
                    uploadId,
                ...
                tx.update(captureSessionRef, {
                    usedAt: session?.usedAt ?? firestore_1.FieldValue.serverTimestamp(),
                    ...
                    lastBoundAt: firestore_1.FieldValue.serverTimestamp(),
                });
                return { uploadId, storagePath, status: 'READY' };
            }
            if (session?.usedAt) {
                ...
                if (evidenceSnap.exists) {
                    if (evidenceSnap.data()?.requestFingerprint !== requestFingerprint) {
                        throw new https_1.HttpsError('failed-precondition', 'The finalized evidence identifier is bound to different evidence.');
                    }
                    return { uploadId, storagePath, status: 'FINALIZED' };
                }
                if (pendingSnap.exists && pendingSnap.data()?.requestFingerprint !== requestFingerprint) {
                    throw new https_1.HttpsError('failed-precondition', 'The reserved evidence grant does not match this capture.');
                }
                if (pendingSnap.exists) {
    ...
    const grant = await config_1.db.runTransaction(async (tx) => {
        ...
        if (pendingSnap.exists && pendingSnap.data()?.requestFingerprint !== requestFingerprint) {
            throw new https_1.HttpsError('failed-precondition', 'The reserved evidence identifier is bound to different evidence.');
        }
        if (pendingSnap.exists) {
            tx.update(pendingRef, { reissuedAt: firestore_1.FieldValue.serverTimestamp(), expiresAt: grantExpiresAt });
        }
...
exports.submitShipping = (0, https_1.onCall)(callOptions, async (request) => {
    const uid = (0, helpers_1.requireUid)(request);
    ...
    await config_1.db.runTransaction(async (tx) => {
        ...
        if (!freshTransaction.exists || !freshPacking.exists || !freshSeal.exists)
            throw new https_1.HttpsError('failed-precondition', 'Shipping evidence changed before shipment could be recorded.');
    ...
    await (0, helpers_1.appendEvent)(input.transactionId, uid, 'SHIPPED', `Seller recorded shipment with ${input.carrier}.`, {
        carrier: input.carrier,
        trackingNumber: input.trackingNumber,
        packingEvidenceId: packingEvidenceRef.id,
        sealEvidenceId: sealEvidenceRef.id,
...
exports.markReceived = (0, https_1.onCall)(callOptions, async (request) => {
    ...
    (0, helpers_1.assertParticipant)(data, uid);
    if (data.buyerId !== uid)
        throw new https_1.HttpsError('permission-denied', 'Only the buyer can mark the item received.');
...
exports.confirmLocalHandoff = (0, https_1.onCall)(callOptions, async (request) => {
    ...
    const completed = await config_1.db.runTransaction(async (tx) => {
        ...
        const record = snap.data();
        (0, helpers_1.assertParticipant)(record, uid);
        if (record.terms.saleType !== 'LOCAL_HANDOFF')
            throw new https_1.HttpsError('failed-precondition', 'This transaction is not a local handoff.');
        if (record.status !== 'TERMS_LOCKED')
            throw new https_1.HttpsError('failed-precondition', 'Lock the terms before confirming the handoff.');
        if (!record.buyerId)
            throw new https_1.HttpsError('failed-precondition', 'A buyer must join before the handoff.');
        ...
        tx.update(ref, {
            ...
            handoffAt: both ? firestore_1.FieldValue.serverTimestamp() : null,
            updatedAt: firestore_1.FieldValue.serverTimestamp(),
        });
        return both;
    });
    await (0, helpers_1.appendEvent)(transactionId, uid, 'HANDOFF_CONFIRMED', completed ? 'Both parties confirmed the local handoff.' : 'A participant confirmed the local handoff.');
    await (0, helpers_1.notifyOtherParticipants)(transactionId, uid, completed ? 'Local handoff confirmed' : 'Handoff confirmation received', completed ? 'Both parties confirmed the item changed hands.' : 'Confirm the handoff in PackProof when it occurs.');
    return { completed };
});
exports.completeTransaction = (0, https_1.onCall)(callOptions, async (request) => {
    const uid = (0, helpers_1.requireUid)(request);
    const { transactionId } = validation_1.transactionIdSchema.parse(request.data);
    const ref = config_1.db.collection('transactions').doc(transactionId);
    const completed = await config_1.db.runTransaction(async (tx) => {
        ...
        const record = snap.data();
        (0, helpers_1.assertParticipant)(record, uid);
        if (!['BUYER_REVIEW', 'DISPUTED'].includes(record.status))
            throw new https_1.HttpsError('failed-precondition', 'The transaction is not ready to complete.');
        ...
        tx.update(ref, { completedBy, status: both ? 'COMPLETED' : record.status, completedAt: both ? firestore_1.FieldValue.serverTimestamp() : null, updatedAt: firestore_1.FieldValue.serverTimestamp() });
        return both;
    });
...
exports.raiseConcern = (0, https_1.onCall)(callOptions, async (request) => {
    const uid = (0, helpers_1.requireUid)(request);
    const input = validation_1.reportSchema.parse(request.data);
    const { ref, data } = await (0, helpers_1.getTransaction)(input.transactionId);
    (0, helpers_1.assertParticipant)(data, uid);
    const reportRef = config_1.db.collection('reports').doc();
    await reportRef.set({ ...input, reporterId: uid, status: 'OPEN', createdAt: firestore_1.FieldValue.serverTimestamp() });
    await ref.update({ status: 'DISPUTED', updatedAt: firestore_1.FieldValue.serverTimestamp() });
    await (0, helpers_1.appendEvent)(input.transactionId, uid, 'CONCERN_RAISED', 'A participant raised a concern. The evidence record remains unchanged.', { reportId: reportRef.id, reason: input.reason });
    await (0, helpers_1.notifyOtherParticipants)(input.transactionId, uid, 'Concern raised', 'The normal completion flow is paused. Review the private transaction record.');
    return { reportId: reportRef.id };
});
exports.blockUser = (0, https_1.onCall)(callOptions, async (request) => {
    const uid = (0, helpers_1.requireUid)(request);
    const targetUserId = String(request.data?.targetUserId ?? '');
    if (!targetUserId || targetUserId === uid || targetUserId.length > 128)
        throw new https_1.HttpsError('invalid-argument', 'Choose another PackProof user.');
    await config_1.db.collection('users').doc(uid).collection('blocks').doc(targetUserId).set({ createdAt: firestore_1.FieldValue.serverTimestamp() });
    return { success: true };
});
exports.registerPushToken = (0, https_1.onCall)(callOptions, async (request) => {
    const uid = (0, helpers_1.requireUid)(request);
    const token = String(request.data?.token ?? '');
    if (!/^ExponentPushToken\[[\w-]+\]$/.test(token) && !/^ExpoPushToken\[[\w-]+\]$/.test(token))
        throw new https_1.HttpsError('invalid-argument', 'Invalid notification token.');
    const previousOwners = await config_1.db.collection('users').where('expoPushToken', '==', token).get();
    await Promise.all(previousOwners.docs.filter((doc) => doc.id !== uid).map((doc) => doc.ref.update({ expoPushToken: firestore_1.FieldValue.delete() })));
    await config_1.db.collection('users').doc(uid).set({ expoPushToken: token, updatedAt: firestore_1.FieldValue.serverTimestamp() }, { merge: true });
    return { success: true };
});
exports.unregisterPushToken = (0, https_1.onCall)(callOptions, async (request) => {
    const uid = (0, helpers_1.requireUid)(request);
    await config_1.db.collection('users').doc(uid).set({ expoPushToken: firestore_1.FieldValue.delete(), updatedAt: firestore_1.FieldValue.serverTimestamp() }, { merge: true });
    return { success: true };
});
