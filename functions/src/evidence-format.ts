...
export const EVIDENCE_MANIFEST_SCHEMA_VERSION = 2 as const;
export const CANONICALIZATION_PROFILE = 'PACKPROOF_JCS_1' as const;
export const BUNDLE_BINDING_PROFILE = 'PACKPROOF_EVIDENCE_BUNDLE_V2' as const;

const HEX_SHA256 = /^[a-f0-9]{64}$/;
const MP4_BRANDS = new Set(['isom', 'iso2', 'mp41', 'mp42', 'avc1', 'M4V ', 'MSNV', 'mp71', 'dash']);

export function detectSupportedMediaType(prefix: Buffer): 'image/jpeg' | 'image/png' | 'video/mp4' | 'application/pdf' | null {
  if (prefix.length >= 4 && prefix[0] === 0xff && prefix[1] === 0xd8 && prefix[2] === 0xff) return 'image/jpeg';
  ...
  if (prefix.length >= 12 && prefix.subarray(4, 8).toString('ascii') === 'ftyp') {
    ...
    for (let offset = 16; offset + 4 <= prefix.length; offset += 4) brands.push(prefix.subarray(offset, offset + 4).toString('ascii'));
...
export function canonicalizeJson(value: unknown, path = '$'): string {
  ...
  if (typeof value === 'object') {
    ...
    return `{${keys.map((key) => {
      ...
      if (item === undefined) throw new TypeError(`${path}.${key} must be omitted or set to an explicit JSON value.`);
 ...
 */
export function createEvidenceBundleSha256(fileSha256: string, manifestSha256: string): string {
...
export function deterministicUploadId(input: {
  transactionId: string;
  uploaderId: string;
  clientEvidenceId: string;
}): string {
  return sha256Hex(Buffer.from(
    `PACKPROOF-UPLOAD-ID\0v1\0${input.transactionId}\0${input.uploaderId}\0${input.clientEvidenceId}`,
    'utf8',
  ));
}
