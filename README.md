...
## Current 0.9.5.0 candidate scope
...
- PackProof Enterprise™ architecture, warehouse domain, Edge protocol, and a simulated single-station Edge runtime (`SOURCE_CHECKED` only). See [`docs/PACKPROOF_ENTERPRISE.md`](docs/PACKPROOF_ENTERPRISE.md).
...
The supplied [`BUILD_REPORT.md`](BUILD_REPORT.md) and [`REMEDIATION_REPORT.md`](REMEDIATION_REPORT.md) are historical 0.2.1 records. They are not evidence that 0.9.5.0 is deployed or runtime-accepted. The current source-gate attestation for the Connect API SHA `a8a1db1` and the follow-on fail-closed grant/lease work is [`docs/releases/BUILD_REPORT_2026-08-15.md`](docs/releases/BUILD_REPORT_2026-08-15.md). That file is also not live-backend, APK, or two-device evidence.
