...
import {
  PackProofApiError,
  pickConnectSession,
  salesforceNamedCredentialCallout,
} from '../force-app/main/default/lwc/packProofProof/api.js';
import { receiveProof } from '../force-app/main/default/lwc/packProofProof/lookup.js';
...
test('Named Credential callout never inlines the API key', () => {
  const options = salesforceNamedCredentialCallout('/v1/connect/sessions?externalOrderId=order-123');
  assert.equal(options.namedCredential, 'PackProof_API');
  assert.equal(options.method, 'GET');
  assert.equal(options.path, '/v1/connect/sessions?externalOrderId=order-123');
  assert.equal(options.authorization, 'Bearer {!$Credential.PackProof_API.api_key}');
  assert.doesNotMatch(options.authorization, /pp_(?:live|sandbox)_/);
});

test('receives Proof through Connect externalOrderId then transaction proof path', async () => {
  const result = await receiveProof({
    candidate: { kind: 'order', value: 'order-123', source: 'case.order_field' },
